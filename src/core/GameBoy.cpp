...
GameBoy::GameBoy()
  ...
  m_SerialIndex = 0;
  memset(&m_PCTrace, 0, sizeof(m_PCTrace));
  m_PCTraceIdx = 0;
...
int GameBoy::NextOpCodeExcute() {
  ...
  // EI delay: takes effect at start of NEXT instruction
  if (m_EIpending) {
    m_EIpending = false;
    m_MasterInterrupt = true;
  }

  // Record PC trace for restart detection
  m_PCTrace[m_PCTraceIdx] = m_programCounter;
  m_PCTraceIdx = (m_PCTraceIdx + 1) % 64;
